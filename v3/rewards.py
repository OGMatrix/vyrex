...
class RotationReward(RewardFunction[AgentID, GameState, float]):
    """
    Context-aware 2v2 rotation reward.

    v3.3 REWRITE: The old version gave a flat 0.8 to the second bot for being
    "between ball and own goal" regardless of context. This created CAMPING:
    when ball was at opponent's goal, the second bot sat at home doing nothing
    while the first bot struggled alone.

    New design: The second bot's ideal position depends on WHERE the ball is:
      - Ball in ATTACKING half → second bot should be in MIDFIELD (support position,
        ready for passes, rebounds, or quick transition). NOT camping at home.
      - Ball in DEFENSIVE half → second bot should be between ball and goal
        (traditional defensive rotation).
      - First bot (closest to ball) → rewarded for being close to ball.

    This creates more fluid, dynamic play where both bots participate in attacks
    and the support bot pushes up when the team is on offense.
    ...
    def get_rewards(self, agents: List[AgentID], state: GameState,
        ...
        for agent in agents:
            ...
            if teammates_closer == 0:
                # I'm closest to ball — I should be engaging. Reward for being close.
                rewards[agent] = max(0.0, 1.0 - (my_ball_dist / 6000.0))
            else:
                # I'm the support player — position depends on ball location
                # Normalize ball Y position to [0, 1] where 0=own goal, 1=opp goal
                field_length = abs(opp_goal_y - own_goal_y)
                if not car.is_orange:
                    # Blue: own goal at -BACK_NET_Y, opp at +BACK_NET_Y
                    ball_field_pct = (ball_pos[1] - own_goal_y) / field_length
                else:
                    # Orange: own goal at +BACK_NET_Y, opp at -BACK_NET_Y
                    ball_field_pct = (own_goal_y - ball_pos[1]) / field_length

                ball_field_pct = max(0.0, min(1.0, ball_field_pct))

                # v3.7: SIMPLIFIED — removed X-centering and speed_factor.
                # v3.6 added X-centering + speed awareness but this created two
                # competing gradient directions that pulled bot AWAY from wall
                # boost pads → 58.6% zero boost in match. The X-penalty punished
                # pathing near walls where 70% of boost pads live. Speed factor
                # added noise without clear behavioral improvement.
                # Now: Y-position only — clean single-axis rotation signal.

                if ball_field_pct > 0.5:
                    # === BALL IN ATTACKING HALF ===
                    # v3.21: Support bot trails ball by ~2500uu — close enough
                    # for passes and rebounds. Old formula placed bot at MIDFIELD
                    # (always y≈0), 3000-5000uu from attacker. User: "passes but
                    # other bot too far behind." New: retreat_dist behind ball,
                    # clamped to avoid retreating past own third.
                    retreat_dist = 2500.0
                    if not car.is_orange:
                        ideal_y = ball_pos[1] - retreat_dist
                        ideal_y = max(ideal_y, -2000.0)  # Don't fall deeper than ~1/3 own half
                    else:
                        ideal_y = ball_pos[1] + retreat_dist
                        ideal_y = min(ideal_y, 2000.0)

                    dist_to_ideal = abs(my_pos[1] - ideal_y)
                    max_dist = field_length * 0.5
                    rewards[agent] = max(0.0, 1.0 - (dist_to_ideal / max_dist)) * 0.7

                else:
                    # === BALL IN DEFENSIVE HALF ===
                    # Between ball and own goal
...
class GroundDribbleReward(RewardFunction[AgentID, GameState, float]):
    ...
    def get_rewards(self, agents: List[AgentID], state: GameState,
        ...
        for agent in agents:
            ...
            # Reward speed toward opponent goal (Y-axis dominant direction)
            goal_y = common_values.BACK_NET_Y if not car.is_orange else -common_values.BACK_NET_Y
            goal_dir = np.sign(goal_y)  # +1 for blue (attack +Y), -1 for orange (attack -Y)
            speed_toward_goal = car.physics.linear_velocity[1] * goal_dir

            # Normalize and clip to [0, 1] — no reward for backward carries
            rewards[agent] = max(0.0, speed_toward_goal / common_values.CAR_MAX_SPEED)
...
class KickoffReward(RewardFunction[AgentID, GameState, float]):
    """
    Role-differentiated kickoff reward for 2v2.

    v3.16: Rewards speed toward ball during kickoff.
    v3.20: ROLE DIFFERENTIATION — only the closest bot goes for ball.

    v3.19 PROBLEM: Both bots rewarded equally for speed toward ball at
    kickoff → both commit → ball goes past both → open net → easy goal.
    User report: "double-committing on kickoff and then getting scored on
    because both of them are in front." Match evidence: double_commit_pct
    rose from 5.7% (v3.18) to 8.5-13.8% (v3.19).

    v3.20 FIX: Determine which bot is closest to ball during kickoff.
      - CLOSEST bot (go-er): rewarded for speed toward ball (unchanged)
      - SECOND bot (support): rewarded for positioning BEHIND the go-er,
        staying in own half. Specifically: gets reward for Y-distance
        toward own goal relative to teammate — teaches "stay back, cover."

    In proper 2v2 kickoffs, one bot goes and one cheats up or defends.
    This creates natural role separation without hardcoding positions.
    ...
    def __init__(self, ball_speed_threshold: float = 100.0,
        ...
        """
        Args:
            ball_speed_threshold: Ball speed below which kickoff state is active.
                Ball starts at 0 speed; first touch typically sends it >1000.
            ball_center_threshold: Maximum distance of ball from center for kickoff.
                Ball spawns at (0, 0, ~93). 200 uu allows for slight drift.
        """
    ...
    def get_rewards(self, agents: List[AgentID], state: GameState,
        ...
        ball_near_center = (abs(ball.position[0]) < self.ball_center_threshold and
                            abs(ball.position[1]) < self.ball_center_threshold and
                            ball.position[2] < 200.0)

        is_kickoff = ball_speed < self.ball_speed_threshold and ball_near_center
        ...
        for team_agents in teams.values():
            ...
            for a in team_agents:
                d = np.linalg.norm(ball.position - state.cars[a].physics.position)
                if d < min_dist:
                    min_dist = d
                    closest = a
        ...
        for agent in agents:
            ...
            if agent in closest_to_ball:
                # === GO-ER: Speed toward ball (original behavior) ===
                ...
                # Reward for being in own half and behind teammate
                own_goal_y = -common_values.BACK_NET_Y if not car.is_orange else common_values.BACK_NET_Y
                car_y = car.physics.position[1]

                # How far back (toward own goal) is the car? [0, 1]
                field_half = abs(own_goal_y)  # BACK_NET_Y
                if not car.is_orange:
                    # Blue: own goal at -Y, further negative = closer to own goal
                    depth = max(0.0, -car_y) / field_half
                else:
                    # Orange: own goal at +Y, further positive = closer to own goal
                    depth = max(0.0, car_y) / field_half

                # Reward for being in own half (depth > 0) with diminishing returns
                rewards[agent] = min(1.0, depth * 1.5) * 0.6  # Cap at 0.6 — less than go-er's max
...
def build_vyrex_reward(config=None):
    ...
    reward_components = [
        ...
        ), rc.boost_change_weight),                                  # 0.20 (v3.20: conditional airborne exempt)
        ...
        (BoostConservationReward(min_ball_height=300.0), rc.boost_conservation_weight),   # 0.04 (v3.20: conditional exempt)
